...
class SyncOrchestrator:
    ...
    async def run_sync(
        ...
        Args:
            ...
            mode: Sync mode - 'delta' (daily, process changed products) or
    ...
    async def _execute_sync(
        ...
        # Load brand mapping from DB (supplier_mapping table)
        phase_start = time.perf_counter()
        ...
        # ── Phase 4: Get products to sync (SQL-level LIMIT) ──
        phase_start = time.perf_counter()
        products_to_sync = list(sync_repo.get_products_for_sync(
            mode=mode, limit=max_products
        ))
        mode_label = "FULL" if mode == "full" else "DELTA"
        if max_products:
            sync_logger.log_progress(
                f"[Phase 4/7] {mode_label} MODE: {len(products_to_sync):,} products "
                f"(capped at {max_products:,})"
            )
        else:
            sync_logger.log_progress(
                f"[Phase 4/7] {mode_label} MODE: {len(products_to_sync):,} products to process"
            )
        ...
        sync_logger.log_progress(
            ...
            f"languages={len(lang_pairs)}: {','.join(languages)})..."
        )
        ...
        # Single semaphore guards ALL API calls (including per-language calls)
        # This prevents unbounded concurrency in multi-language mode
        # (e.g., 40 products x 9 languages = 360 calls without this)
                ...
                if multi_lang:
                    async def _fetch_all_langs(sp: SyncProduct):
                        ...
                        async def _fetch_one_lang(short_code: str, lid: int):
                            async with api_semaphore:  # Each language call is bounded
                                try:
                    ...
                    # Single-language: each API call guarded by semaphore
    ...
    async def _sync_single_product(
        ...
        # Step 1: Match with Icecat (apply brand mapping)
